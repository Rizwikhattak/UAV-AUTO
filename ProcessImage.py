...
def extract_frames(data, output_folder, frames_per_second=5):
    ...
    start_location = LocationPin.query.filter_by(id=data['location_pin_id']).first()
    end_location = LocationPin.query.filter_by(id=data['end_location_pin_id']).first()
    ...
    # Create damaged frames folder

    def process_and_save(selected_frames, second):
        """Process selected frames, detect damage, and save results."""
    ...
    while True:
        ...
        # Append the frame to the current second's list
