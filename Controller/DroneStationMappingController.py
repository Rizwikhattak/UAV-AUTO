...
class DroneStationMappingController():
    ...
    def insert_drone_sation_mapping(data):
        try:
            ...
            if drone and station:
                StationController.increment_num_drones(data['station_id'])
                db.session.commit()
    ...
    def update_drone_station_mapping(data):
        try:
            ...
            if drone_station_mapping:
                StationController.decrement_num_drones(drone_station_mapping.station_id)
                ...
                db.session.commit()
                StationController.increment_num_drones(drone_station_mapping.station_id)
                return {
                    "id": drone_station_mapping.id,
                    "drone_id": drone_station_mapping.drone_id,
                    "station_id": drone_station_mapping.station_id,
    ...
    def delete_drone_station_mapping_by_id(id):
        try:
            ...
            if station:
                StationController.decrement_num_drones(station.id)
                db.session.commit()
            if drone_station_mapping:
